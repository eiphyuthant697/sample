...
//  }
  function cp_uploadProofImage(id,status)
 {  var proof = document.getElementById("img_uploadP_"+id).getAttribute("src");
    document.location = 'admin.php?page=<?php echo $this->menu_parameter; ?>&anonce=<?php echo $nonce; ?>&cal=<?php echo intval($_GET["cal"]); ?>&list=1&status='+status+'&pf='+proof+'&lu='+id+'&r='+Math.random( );
 }
  ...
  function openMediaLibrary(id) {
        var upload = wp.media({
            ...
            .on('select', function() {
                ...
                var previewImage = document.getElementById("img_uploadP_"+ id);
                //console.log(previewImage);
                jQuery(previewImage).attr('src', attach.url);
                var previewLabel = document.getElementById("label_uploadP_"+ id);
                jQuery(previewLabel).html('preview');
            })
            .open();
    }
</script>
...
<div class="ahb-section-container search-message">
...
<table border="0" style="width:100%;" class="ahb-orders-list" cellpadding="10" cellspacing="10">
    ...
    <tr>
      ...
      <th width="130"><?php _e('Deliver Proof','appointment-hour-booking'); ?></th>
    ...
    <tbody id="the-list">
     <?php for ($i=($current_page-1)*$records_per_page; $i<$current_page*$records_per_page; $i++) if (isset($events[$i])) {
              ...
              for($k=0; $k<count($posted_data["apps"]); $k++)
                  ...
                  { 
                    if ($posted_data["apps"][$k]["cancelled"] == 'Delivered')
                    {
                        $delivered = true;
                    }
                      $cancelled = true;
     ...
     ?>
      <tr class='<?php if ($cancelled) { ?>cpappb_cancelled <?php } ?><?php if ($delivered) { ?>cpappb_delivered <?php } ?><?php if (($i%2)) { ?>alternate <?php } ?>author-self status-draft format-default iedit' valign="top">
        ...
        <td><?php
                ...
                echo $data;
            ?></td>
        <td align="center"><p style="color:#006799;font-weight:bold;margin-top:0;" id="label_uploadP_<?php echo $events[$i]->id; ?>">Preview</p><img id="img_uploadP_<?php echo $events[$i]->id; ?>" class="img-uploadP" src="<?php echo $posted_data["payment_type"]; ?>" style="height:100px;width:auto;margin-bottom:10px;"><input class="button img-upload" type="button" id="uploadMediaLibrary_<?php echo $events[$i]->id; ?>" name="uploadMediaLibrary_<?php echo $events[$i]->id; ?>" value="Select Proof Image" onclick="openMediaLibrary(<?php echo $events[$i]->id; ?>)"/></td>
        <td class="cpnopr" style="text-align:center;">
        <input class="button" type="button" name="caldelete_<?php echo $events[$i]->id; ?>" value="Upload Proof Image" onclick="cp_uploadProofImage(<?php echo $events[$i]->id; ?>,<?php echo (!empty($posted_data["paid"]) && $posted_data["paid"]?'0':'1'); ?>);" />
          ...
          ?>
          <?php //if ($delivered) {, $delivered disable?>
          <nobr><?php $this->render_status_box('statusbox'.$events[$i]->id, $status); ?><input class="button" type="button" name="calups_<?php echo $events[$i]->id; ?>" value="Update Status" onclick="cp_UpsItem(<?php echo $events[$i]->id; ?>);"/></nobr><?php //} ?>
        </td>
      </tr>
     <?php } ?>
    </tbody>
