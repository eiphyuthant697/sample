    ...
    $.fbuilder.controls['ffields'].prototype, {
        ...
        getAvailablePartialSlots: function(d, part, s) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                //if (me.avoidOverlaping && (data[i].quantity + me.quantity_selected > capacity_service || (data[i].service.length == 0 || (data[i].service.length && data[i].service[0] != s))) ||            (!me.avoidOverlaping && (data[i].quantity + me.quantity_selected > capacity_service && (typeof data[i].serviceindex === 'undefined' || data[i].serviceindex == s)))) {
                //!me.avoidOverlaping && 
                if ((data[i].service.length == 0 || (data[i].service.length && data[i].service[0] != s)) || (data[i].quantity + me.quantity_selected > capacity_service && (typeof data[i].serviceindex === 'undefined' || data[i].serviceindex == s))) {
        ...
        after_show: function() {
            ...
            function onChangeDateOrService(d) {
                ...
                function getSlots(d) {
                    ...
                    document.getElementById("checkCountry").addEventListener('change', (event) => {
                        checkCountry = event.target.value;
                        //console.log(checkCountry);
                    });

                    s.html("<div class=\"slots\" d=\"" + d + "\"><span class=\"currentDate\">" + $.datepicker.formatDate(me.dateFormat, $.datepicker.parseDate("yy-mm-dd", d)) + "</span><br / > " + str + " </div>");


                    $.post(
                        "../wp-content/themes/businessbiz/tracking-code-with-country-and-date.php", { date: d, country: checkCountry },
                        function(data) {
                            $('input#fieldname7_1').val(data);
                        }
                    );
                    if (before != "" && before != d) {
                    ...
                    $('.usedSlots' + me.name + ' a.cancel').off("click").on("click", function() {
                        ...
                        var find = false;
                        for (var i = 0;
                            (i < me.usedSlots[d].length && !find); i++)
                        ...
                        var find = false;
                        for (var i = 0;
                            (i < me.allUsedSlots.length && !find); i++)
            ...
            e.datepicker({
                numberOfMonths: parseFloat(me.numberOfMonths),
            ...
            if (me.maxDate !== "")
                try {
                    me.getMaxDate = $.datepicker._getMinMaxDate(e.data('datepicker'), 'max');
                    me.getMaxDate.setHours(23, 59, 59, 999);
                } catch (e) {}
            try {
                if (me.defaultDate !== "")
                    ...
                    if (me != null) {
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                me.loadOK = true;
                                for (var i = 0;
                                    (i < me.allUsedSlots.length && !overlapping); i++) {
