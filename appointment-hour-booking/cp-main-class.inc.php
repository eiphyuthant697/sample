...
class CP_AppBookingPlugin extends CP_APPBOOK_BaseClass {
    ...
    protected $paid_statuses = array('Pending','Cancelled','Shipping','Delivered');
    ...
    {
       // $disable = '';
        //if($delivered) $disable = 'disabled', $delivered;'.$disable.'
    ...
    // public function update_proof_image($id, $proof, $status)
    // {
    //     global $wpdb;
    //     $events = $wpdb->get_results( $wpdb->prepare('SELECT * FROM `'.$wpdb->prefix.$this->table_messages.'` WHERE id=%d', $id) );
    //     $posted_data = unserialize($events[0]->posted_data);        
    ...
    //     for($k=0; $k<$countapps; $k++)
    //     {
    //         $posted_data["paid"] = $status;
    //          $posted_data["payment_type"] = $proof;         
    //     }
    //     $posted_data = serialize($posted_data);
    //     $wpdb->update ( $wpdb->prefix.$this->table_messages, array( 'posted_data' => $posted_data ), array( 'id' => $id ));
    //     do_action( 'cpappb_update_proof_image', $id, $proof ); 
    // }
    ...
    public function filter_list($atts) {
        ...
        if (!is_admin())
        {
            wp_enqueue_style('cpapp-publicstyle', plugins_url('css/stylepublic.css', __FILE__));
             wp_enqueue_style('inltelstyle', 'https://cdnjs.cloudflare.com/ajax/libs/intl-tel-input/14.0.7/css/intlTelInput.css');
    ...
    function insert_public_item() {
        ...
        if (CP_APPBOOK_DEFER_SCRIPTS_LOADING)
        {
            ...
            // wp_enqueue_script('inltel-js', plugins_url('js/intlTelInput'.'.js', __FILE__));
            ...
            } else <?php } ?>
            {
                ...
                if (cpefb_error==0)
                {
                    if (!apphboverbooking_handler<?php echo $this->print_counter-1; ?>)
                    {
                        ...
                        $dexQuery("#<?php echo $this->prefix; ?>_pform<?php echo '_'.$this->print_counter; ?>").find(".avoid_overlapping_before").not(".ignore,.ignorepb").removeClass("avoid__before").removeClass("valid").addClass("avoid_");
...
// print_r($item);exit;
    ...
    function ready_to_go_reservation($itemnumber, $payer_email = "")
    {
        ...
        {
            // $explodeData = explode(": ", $buffer_A);
            // $actualtrackCode = explode(" ", $explodeData[6]);
            // $trackCode = explode(PHP_EOL, $actualtrackCode[0]);
            $code = $params["fieldname7"];
            $buffer .= "I agree to Privacy Policy of UK Cargo.\n\n";
            $tracking = 'Tracking with this <a href="http://web-dev.88175848-86-20200805092356.webstarterz.com/uk-cargo/track-form/?tracking=' . $code . '&mail='.$myrows[0]->notifyto.'" title="Verify">Link</a>';
            //$message = str_replace('%track%',$tracking,$message);
            $buffer .= $tracking;
        ...
        if (($destination_to != '' || $payer_email != '') && 'true' == $this->get_option('cu_enable_copy_to_user', CP_APPBOOK_DEFAULT_cu_enable_copy_to_user))
        {
            ...
            $mail_code = unserialize($myrows[0]->posted_data);
            $track_code = $mail_code["fieldname7"];
            
            $message .= "\nYou agree to Our Privacy Policy.\n\n";
            
            $copy_message = 'Tracking with this <a href="http://web-dev.88175848-86-20200805092356.webstarterz.com/uk-cargo/track-form/?tracking=' . $track_code . '&mail='.$myrows[0]->notifyto.'" title="Verify">Link</a>';
