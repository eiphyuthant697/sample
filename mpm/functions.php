...
function foodica_enqueue_scripts() {
	...
	wp_enqueue_style( 'foodica-bootstrap-style', 'https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/css/bootstrap.min.css' );
	...
	wp_enqueue_style( 'foodica-style-fontawesome', get_template_directory_uri() . '/assets/css/all.css', array( 'foodica-style' ), '5.9.0' );
    
	wp_enqueue_style( 'foodica-google-font-default', '//fonts.googleapis.com/css2?family=Sahitya:wght@400;700&display=swap' );

    wp_enqueue_style('foodica-google-font-gb', '//fonts.googleapis.com/css2?family=EB+Garamond:wght@400;500;600;700&amp;display=swap');

    wp_enqueue_style('foodica-google-font-jp', '//fonts.googleapis.com/css2?family=Noto+Sans+JP:wght@300;400;500;700&display=swap');

    wp_enqueue_style( 'foodica-google-font-opensans', '//fonts.googleapis.com/css2?family=Open+Sans:ital,wght@0,400;0,600;1,700&display=swap' );

    wp_enqueue_style( 'foodica-google-font-montserrat', '//fonts.googleapis.com/css2?family=Montserrat:wght@300;400;500;600;700;800&display=swap' );

    wp_enqueue_style( 'foodica-google-font-lato', '//fonts.googleapis.com/css2?family=Lato:wght@700&display=swap' );
    ...
    // wp_enqueue_script( 'foodica-jquery-3', get_template_directory_uri() . '/assets/js/jquery-3.4.1.min.js', array( 'jquery' ), '3.4.1', true );

    //wp_enqueue_script( 'foodica-wow', get_template_directory_uri() . '/assets/js/wow.min.js');
...
echo '<style>
    #menu-comments, #toplevel_page_sharethis-general, #menu-plugins, #menu-tools, #menu-settings, #toplevel_page_edit-post_type-acf-field-group, #toplevel_page_cptui_main_menu
 	{
 		display: none!important;
 	}
#menu-posts-questions li:nth-child(3), .post-type-questions a.page-title-action, .acf-input .quicktags-toolbar {
    display: none;
}
.acf-input textarea {
...
function department_content_home($attr, $content, $tag){
    get_template_part('content', 'department_post');
}
add_shortcode('department_content_shortcode', 'department_content_home');

function operation_content_home($attr, $content, $tag){
    get_template_part('content', 'operation_time');
}
add_shortcode('operation_content_shortcode', 'operation_content_home');

function news_content_home($attr, $content, $tag){
    get_template_part('content', 'home_news');
}
add_shortcode('news_content_shortcode', 'news_content_home');

function feature_content_home($attr, $content, $tag){
    get_template_part('content', 'our_feature');
}
add_shortcode('feature_content_shortcode', 'feature_content_home');

function department_service_home($attr, $content, $tag){
    get_template_part('content', 'department_service');
}
add_shortcode('department_service_content_shortcode', 'department_service_home');

function about_doctor_content($attr, $content, $tag){
    get_template_part('content', 'abt_doctor');
}
add_shortcode('doctor_profile_content_shortcode', 'about_doctor_content');

function about_feature_content($attr, $content, $tag){
    get_template_part('content', 'abt_feature');
}
add_shortcode('feature_image_thumbnail_slider', 'about_feature_content');

function news_page_content($attr, $content, $tag){
	get_template_part('content', 'news_page');
}
add_shortcode('news_page_shortcode','news_page_content');

function patient_section($attr, $content, $tag){
	get_template_part('content', 'patient');
}
add_shortcode('patient_shortcode','patient_section');

function questions_section($attr, $content, $tag){
    get_template_part('content', 'questions');
}
add_shortcode('questions_content_shortcode','questions_section');