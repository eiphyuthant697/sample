...
if( !class_exists('FPD_Designs') ) {
	...
	class FPD_Designs {
		...
		public static function get_category_data( $category_id ) {
			...
			$row = $wpdb->get_row("SELECT ID, title, options, thumbnail, parent_id FROM ".FPD_DESIGNS_TABLE. " WHERE ID=$category_id", ARRAY_A);
		...
		public static function get_category_designs( $category_id ) {
			...
			if( fpd_table_exists(FPD_DESIGNS_TABLE) ) {
				...
				$row = $wpdb->get_row("SELECT designs FROM ".FPD_DESIGNS_TABLE. " WHERE ID=$category_id");
